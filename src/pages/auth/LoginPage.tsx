...
const schema = z.object({
  email: z.string().min(2, "Nhập email đăng nhập"),
  ...
  return (
    <div>
      ...
      <p className="text-sm mb-7" style={{ color: "var(--text-3)" }}>
        Chào mừng trở lại FNB HUB
      </p>

      <form
        onSubmit={handleSubmit((d) =>
          login(d, { onSuccess: () => navigate(from, { replace: true }) })
        )}
        className="space-y-4"
      >
        <Field
        ...
        <Button
          type="submit"
          disabled={isPending}
          className="w-full mt-2 h-11 text-base font-bold rounded-xl"
